...
export const Template = (args, { argTypes }) => ({
  ...
  components: { Input },
  template: '<Input :type="type" :variant="variant" :size="size" :label="label" :hideLabel="hideLabel" :placeholder="placeholder" :value="value" :id="id" :min="min" :max="max" :step="step" :message="message" :messageType="messageType" />',
})
