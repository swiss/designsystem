      ...
      <div>
        ...
        <Modal uuid="${args.uuid}"  title="${args.title}" triggerElements="${args.triggerElements}" layout="${args.layout}">
          <template v-slot:body>
            <Card type="default">
              <template v-if="${args.body == 'WithDemoText'}" v-slot:body>
                <p>
              ...
              <template v-else="${args.body == 'WithDemoShare'}" v-slot:body>
                <div>
...
See an example for the `Sharebar` on the [detail page simple](?path=/docs/pages-detail-page-simple--example). **(Please view in full-width tab to have the final behaviour)**
...
**uuid**: when several modals are present on a page, they should have an uuid, for accessibility purpose.

**title**: a title placed on top of the modal. For accessibility purpose, this title should always be delivred.
