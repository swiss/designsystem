import { Meta, Story, Canvas } from '@storybook/addon-docs/blocks'
import Tabs from '../../ch/demo/Tabs.vue'
import TabsLarge from '../../ch/demo/TabsLarge.vue'

<Meta
  title="Components/Tabs"
  component={Tabs, TabsLarge}
  argTypes={{
  }}
/>

export const Template = (args, { argTypes }) => ({
  props: Object.keys(argTypes),
  components: { Tabs },
  template: '<Tabs />',
});

export const TemplateLarge = (args, { argTypes }) => ({
  props: Object.keys(argTypes),
  components: { TabsLarge },
  template: '<TabsLarge />',
});

# Tabs
Tabs are used to group related content. They can also be used to switch between views.
<Canvas>
  <Story
    name="Example"
    args={{
      variant: 'default',
    }}
  >
    {Template.bind({})
  }</Story>
</Canvas>

# Overflowing Tabs
While it is not advised to use more than 3 tabs, it can happen that the tabs are too large to fit in the available space. In this case, the tabcontainer can be made to overflow.
<Canvas>
  <Story
    name="LargeExample"
    args={{
      variant: 'default',
    }}
  >
    {TemplateLarge.bind({})
  }</Story>
</Canvas>

<a href="?path=/story/components-tabs--example">
  Go to the Canvas Tab
</a>

---

<a href="?id=components-tabs--example" target="_blank">
  Open page in full width in a new tab
</a>
