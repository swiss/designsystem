...
## Minimal hero for detail pages

**The `.hero.hero--minimal` section is used on [detail pages](?path=/docs/pages-test-hero-minimal--example) where the hero only contains a title.**

This variant can contain only a title.

<Canvas>
  <Story
    name="Detail page - minimal"
    args={{
      type: 'minimal',
      metaInfos: undefined,
      authors: undefined
